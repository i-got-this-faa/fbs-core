...
- *Milestone:* Server boots, connects to SQLite, and unit tests can write atomic files to disk.
...
- *Milestone:* A client can authenticate with a Bearer token and upload/download a simple file.
...
- *Milestone:* `aws s3 ls` works against the server. Management API endpoints are ready for UI consumption.
